...
export class VisibilityHelper {
    ...
    public static evaluateVisibility(visibilityRule: IVisibilityRule, dependentInputs: Map<string, AdoInput>): boolean {
        ...
        for (let i = 0, len = visibilityRule.predicateRules.length; i < len; i++) {
            ...
            let dependentInput: AdoInput = dependentInputs.get(predicateRule.inputName);
            if (dependentInput) {
                let isInputVisible = dependentInput.isVisible;
                if (!isInputVisible) {
                    result = this._evaluate(result, isInputVisible, visibilityRule.operator);
                } else {
                    let predicateResult = this._getPredicateResult(predicateRule, dependentInput.value?.toString());
